...
updated by helmuthb on 2015 may 15
"""
...
ANNOUNCEMENT_TPL = (
    'Last chance to attend! The following conferences are nearly sold out: %s')
FEATUREDSPEAKER_TPL = (
    'Speaker %s is amazing, giving the following sessions: %s')
...
# - - - Speaker objects - - - - - - - - - - - - - - - - -
    ...
    def _createSpeakerObject(self, request):
        ...
        # generate Profile Key based on user ID, use as parent
        p_key = ndb.Key(Profile, user_id)
        data['parent'] = p_key
    ...
    def getSpeaker(self, request):
        ...
        if speaker.key.kind() != "Speaker":
            raise endpoints.NotFoundException(
                'Key does not belong to speaker: %s'
                % request.websafeSpeakerKey)
    ...
    def getSpeakersCreated(self, request):
        ...
        # return set of SpeakerFormOut objects per Speaker
        return SpeakerForms(
...
# - - - Session objects - - - - - - - - - - - - - - - - -
    ...
    @endpoints.method(
        SESS_GET_BY_SPEAKER, SessionForms,
        path='conference/{websafeConferenceKey}/bySpeaker/{websafeSpeakerKey}',
        http_method='GET',
        name='getConferenceSessionsBySpeaker')
    def getConferenceSessionsBySpeaker(self, request):
        """Get all conference sessions with a specified speaker"""
        # get the conference
        conf = ndb.Key(urlsafe=request.websafeConferenceKey)
        # is it really a conference key?
        if conf.kind() != 'Conference':
        ...
        if speaker.kind() != 'Speaker':
            raise endpoints.BadRequestException(
                'Provided speaker key is not a Speaker')
        # is the speaker existing?
        if speaker.get() is None:
            raise endpoints.NotFoundException(
                'Speaker not found')
...
# - - - Problem 3 - - - - - - - - - - - - - - - - - - - - - - - -
    ...
    @endpoints.method(
            CONF_GET_REQUEST, SessionForms,
            path='conference/{websafeConferenceKey}/queryproblem/solution1',
            http_method='GET', name='queryProblemSolution1')
    def queryProblemSolution1(self, request):
        """First solution of query problem using IN operator."""
        # get the conference
        conf = ndb.Key(urlsafe=request.websafeConferenceKey)
        # is it really a conference key?
        if conf.kind() != 'Conference':
        ...
        # get all sessions in the conference
        sessions = Session.query(ancestor=conf)
        # and filter by start time
        sessions = sessions.filter(
            Session.startTime < datetime.strptime("19:00", "%H:%M").time())
        # and filter by typeOfSession - using IN query to avoid inequality
        sessions = sessions.filter(
    ...
    @endpoints.method(
            CONF_GET_REQUEST, SessionForms,
            path='conference/{websafeConferenceKey}/queryproblem/solution2',
            http_method='GET', name='queryProblemSolution2')
    def queryProblemSolution2(self, request):
        """First solution of query problem using python loop."""
        # get the conference
        conf = ndb.Key(urlsafe=request.websafeConferenceKey)
        # is it really a conference key?
        if conf.kind() != 'Conference':
